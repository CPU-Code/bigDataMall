...
insert overwrite table dwd_trade_cancel_detail_inc
    partition (dt)
select od.id,
       order_id,
       user_id,
       ...
       split_coupon_amount,
       split_total_amount,
       date_format(canel_time, 'yyyy-MM-dd') dt
from (
         select data.id,
                data.order_id,
                data.sku_id,
...
show partitions dwd_trade_cancel_detail_inc;
...
select id,
       ...
       split_coupon_amount,
       split_total_amount
from dwd_trade_cancel_detail_inc
where dt = '2020-06-14'
